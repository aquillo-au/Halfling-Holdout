...
def tick_game args
  ...
  args.state.enemies ||= [
    { x: 10, y: 10, type: :goblin, tile_key: :B, hp: BADDIES[:goblin].hp, atk: BADDIES[:goblin].atk},
    { x: 15, y: 20, type: :rat, tile_key: :R, hp: BADDIES[:rat].hp, atk: BADDIES[:rat].hp }
  ]
  ...
  if player_moved
    ...
    args.state.enemies.each do |enemy|
      distances = []
      allies.each do |ally|   
        distances << proximity_to_target(enemy, ally)
      end
      index = distances.index(distances.min)
      new_spot = FindPath.new(enemy, allies[index], args.state.walls).find_me
...
def proximity_to_target(me, target)
  distance_x = (me.x - target.x).abs +distance_y = (me.y - target.y).abs
end
