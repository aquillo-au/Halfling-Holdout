...
def tick_game args
  ...
  args.state.enemies ||= [
    { x: 10, y: 10, type: :goblin, tile_key: :B },
    { x: 15, y: 20, type: :rat, tile_key: '🐀' }
  ]

  args.state.info_message ||= "#{BADDIES.goblin.hp}"
  ...
  if player_moved
    ...
    if !found_enemy
      ...
      type = found_enemy.type
      args.state.info_message = "You ran into a #{found_enemy.type} with #{BADDIES.type.hp}"
    end
  end
