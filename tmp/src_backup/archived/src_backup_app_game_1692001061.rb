...
def gameplay_tick(args)
    ...
    if player_moved
      ...
      args.state.goodies.each do |goody|
        ...
        if nearest_target[1]
          message = nearest_target[1]
          new_spot = BasicPath.new(goody, nearest_target, args.state.walls).move_step
        else
          message = nearest_target
          new_spot = BasicPath.new(goody, nearest_target, args.state.walls).random_direction
        end
        blocking_friend = args.state.allies.find do |a|
