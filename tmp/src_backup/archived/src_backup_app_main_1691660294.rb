...
def tick_game args
  ...
  if player_moved
    ...
    if !found_enemy && !found_wall
      ...
      args.state.info_message = "You can't move through walls!"
    end
  end
  ...
  # render label stuff
  args.outputs.labels << [border_x, border_y - 9, "Current player location is: #{args.state.player.x}, #{args.state.player.y} you have #{args.state.player.hp} HP left"]
  args.outputs.labels << [border_x, border_y + 24 + border_size, args.state.info_message]
end
