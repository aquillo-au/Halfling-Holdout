...
def gameplay_tick(args)
    ...
    if player_moved
      ...
      args.state.goodies.each do |goody|
        ...
        if (blocking_friend || found_wall)
          ...
          args.state.combat_log << other_combat(goody, blocking_opponent).flatten
          args.state.enemies.reject! { |e| e.dead }
      ...
      args.state.enemies.each do |enemy|
        ...
        if args.state.player.x == new_spot.x && args.state.player.y == new_spot.y
          args.state.combat_log << other_combat(enemy, args.state.player).flatten
        elsif blocking_friend
          #don't move
        elsif args.state.hotpot.x == new_spot.x && args.state.hotpot.y == new_spot.y
          args.state.combat_log << other_combat(enemy, args.state.hotpot).flatten
        elsif blocking_opponent
          args.state.combat_log << other_combat(enemy, blocking_opponent).flatten
        else
          enemy.x = new_spot.x
          enemy.y = new_spot.y
        end
    ...
    args.outputs.labels << [border_x + 10, border_y - 10, "Current player location is: #{args.state.player.x}, #{args.state.player.y} you have #{args.state.player.hp} HP left"]
    args.outputs.labels << [border_x + 10, border_y + 35 + border_size, args.state.info_message]
    args.state.combat_log = args.state.combat_log.last(10)
    args.state.combat_log.each_with_index do |log, index|
      args.outputs.labels << [810, (670 - (index*20)) , "#{log}", 0.1,]
    end
  ...
  def check_arrows(args)
    args.state.arrows.each do |arrow|
      ...
      if blocking_friend || found_wall || !still_in_map?(arrow.x, arrow.y)
        ...
        args.state.combat_log << your_arrow(blocking_opponent).flatten
        arrow.dead = true
      end
    end
