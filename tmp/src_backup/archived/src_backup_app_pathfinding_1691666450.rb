class FindPath
  ...
  def initialize (enemy, player)
    ...
    @target = [args.state.player.x, args.state.player.y]
  ...
  def calc_a_star
    ...
    @a_star.came_from[@star] = nil
    @a_star.cost_so_far[@star] = 0
    @a_star.frontier << @star

    # Until there are no more cells to explore from or the search has found the target
    until @a_star.frontier.empty? or @a_star.came_from.key?(@target)
      ...
      adjacent_neighbors(current_frontier).each do | neighbor |
        ...
        unless a_star.came_from.key?(neighbor) or args.state.walls.key?(neighbor)
    ...
    if a_star.came_from.key?(@target)
