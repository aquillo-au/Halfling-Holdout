class FindPath
  ...
  def calc_@star
    ...
    @star.came_from[@star] = nil
    @star.cost_so_far[@star] = 0
    @star.frontier << @star

    # Until there are no more cells to explore from or the search has found the target
    until @star.frontier.empty? or @star.came_from.key?(@target)
      # Get the next cell to expand from
      current_frontier = @star.frontier.shift

      # For each of that cells neighbors
      adjacent_neighbors(current_frontier).each do | neighbor |
        # That have not been visited and are not walls
        unless @star.came_from.key?(neighbor) or args.state.walls.key?(neighbor)
          # Add them to the frontier and mark them as visited
          @star.frontier << neighbor
          @star.came_from[neighbor] = current_frontier
          @star.cost_so_far[neighbor] = @star.cost_so_far[current_frontier] + 1
        end
      end
      ...
      @star.frontier = @star.frontier.sort_by {| cell | proximity_to_star(cell) }
      @star.frontier = @star.frontier.sort_by {| cell | @star.cost_so_far[cell] + greedy_heuristic(cell) }
    end

    # If the search found the target
    if @star.came_from.key?(@target)
      # Calculate the path between the target and star
      @star_calc_path
    end
  end

  # Calculates the path between the target and star for the @star search
  # Only called when the @star search finds the target
  def @star_calc_path
    # Start from the target
    endpoint = @target
    # And the cell it came from
    next_endpoint = @star.came_from[endpoint]
...
#   $@star_algorithm ||= @star_Algorithm.new
#   $@star_algorithm.args = args
#   $@star_algorithm.tick
# end
