...
#       args.audio[:music] = { input: "sounds/flight.ogg", looping: true }
    ...
    args.outputs.sprites << SpriteGrid.new.tile(PADDING_X + 27 * DESTINATION_TILE_SIZE, PADDING_Y + 20 * DESTINATION_TILE_SIZE,:v)
    args.outputs.sprites << SpriteGrid.new.tile(PADDING_X + 31 * DESTINATION_TILE_SIZE, PADDING_Y + 20 * DESTINATION_TILE_SIZE,:c)
    labels << {
      x: 325,
      y: 380,
      text: "Guard | Villager | Cook",
    }
    args.outputs.sprites << SpriteGrid.new.tile(PADDING_X + 1 * DESTINATION_TILE_SIZE, PADDING_Y + 16 * DESTINATION_TILE_SIZE,:r)
    args.outputs.sprites << SpriteGrid.new.tile(PADDING_X + 4 * DESTINATION_TILE_SIZE, PADDING_Y + 16 * DESTINATION_TILE_SIZE,:g)
    labels << {
      x: 125,
      y: 333,
      text: "<-- The Baddies",
