...
def gameplay_tick(args)
    ...
    args.state.goodies ||= [
      { x: 22, y: 14, type: GOODIES[:guard].name, tile_key: GOODIES[:guard].tile_key, hp: GOODIES[:guard].hp, atk: GOODIES[:guard].atk}
    ]
    allies = [args.state.player, args.state.hotpot, args.state.goodies.flatten]
